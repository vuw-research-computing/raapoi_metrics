...
job_data_str = subprocess.run(['sacct', '-S', start_date_str, '-E', end_date_str, '--parsable2', '--allusers', '--format=User,jobid,Elapsed,Timelimit,Start,NNodes,NCPUS,NTasks,MaxRSS,MaxVMSize,Partition,ReqCPUS,AllocCPUS,TotalCPU,CPUtime,ReqMem,State,End,Account'], stdout=subprocess.PIPE).stdout.decode('utf-8')
...
for user in usernames:
    user_jobs_data = all_jobs_data[all_jobs_data['User'] == user]
    
    if not user_jobs_data.empty:
        print(user, end='')
        t0 = time.time()
